...
@section('title', 'Create Stock Write-off')
...
<div class="container-fluid">
    <div class="d-flex justify-content-between align-items-center mb-3">
        <h2 class="text-dark">Create Stock Write-off</h2>
        <a href="{{ url('store/stock/writeoff') }}" class="btn btn-secondary btn-sm">View Write-off History</a>
    </div>
        ...
        <table class="table table-bordered table-sm text-center mb-0" id="itemsTable">
            <thead class="thead-light">
                <tr>
                    ...
                    <th>Available Qty</th>
                    <th style="min-width:110px">Write-off Qty <span class="text-danger">*</span></th>
                    <th style="min-width:140px">Reason <span class="text-danger">*</span></th>
                    <th>Action</th>
                </tr>
            </thead>
    ...
    <div class="text-right">
        <button type="button" id="submitWriteoff" class="btn btn-primary">Submit Write-off</button>
    </div>
</div>
...
$(document).ready(function () {
    ...
    // since inhouse products aren't stock-tracked per store and have no
    // purchase_request batch to deduct from.
    ...
    function resetRowFrom(id, field) {
        ...
        $(`#avail_qty_${id}`).text('–');
        $(`#writeoff_qty_${id}`).val('').removeAttr('max');
    }
    ...
    function loadPricesForPack(id, productId, packId, brandId) {
        loadProductOptions(function (options) {
            ...
            // a write-off always deducts from one specific batch.
    ...
    function applyPriceSelection(id, opt) {
        ...
        $(`#writeoff_qty_${id}`).attr('max', avail);
        document.getElementById(`writeoff_qty_${id}`)?.focus({ preventScroll: true });
    }
    ...
    function addRow() {
        ...
        const row = `
            <tr id="row_${id}">
                ...
                <td>
                    <input type="number" class="form-control writeoff-qty" id="writeoff_qty_${id}" data-row="${id}" min="1" step="1" placeholder="0">
                </td>
                <td>
                    <select class="form-control" id="reason_select_${id}" data-row="${id}">
    ...
    $('#submitWriteoff').on('click', function () {
        const items = [];
        let valid = true;
        ...
        $('#itemsBody tr').each(function () {
            ...
            const qty   = parseFloat($(`#writeoff_qty_${rowId}`).val()) || 0;
            const reason = $(`#reason_select_${rowId}`).val();

            if (!sel.val()) { valid = false; Swal.fire('Validation', 'Please select a product for every row.', 'warning'); return false; }
            if (qty <= 0)   { valid = false; Swal.fire('Validation', 'Write-off qty must be > 0.', 'warning'); return false; }
            const avail = parseFloat(opt.data('avail-qty')) || 0;
            if (qty > avail){ valid = false; Swal.fire('Validation', `Write-off qty exceeds available stock for ${opt.data('product-name')}.`, 'warning'); return false; }
