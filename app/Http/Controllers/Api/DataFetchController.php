...
class DataFetchController extends Controller
{
    public function dataFetch()
    {
        try {
            ...
            $remoteDatastore_assign = DB::connection('remote_mysql')->table('store_assign')->get();
            ...
            $remoteDatapurchase_request = DB::connection('remote_mysql')->table('purchase_request')->get();
            foreach ($remoteDatapurchase_request as $key => $value) {
                $getStoreAssign =  DB::table('purchase_request')->where([ 'store_assign_id' => $value->store_assign_id,
                'brand_id' => $value->brand_id,
                'product_id' => $value->product_id])->first();
